...
import { buildProfessionalEmailHtml } from './emailTemplates';
...
export async function sendLogisticsEmail({ recipientEmail, requestCode, cartItems, preferences, greeting }: SendLogisticsEmailParams): Promise<SendLogisticsEmailResult> {
  ...
  const serializedTemplate = typeof preferences.emailTemplate === 'string'
    ? preferences.emailTemplate
    : templateElementsToString(preferences.emailTemplate);
  const hasCustomTemplate = serializedTemplate.trim().length > 0;
  const htmlBody = hasCustomTemplate
    ? renderEmailTemplate(preferences.emailTemplate, emailContext, 'html')
  ...
  const textBody = hasCustomTemplate
    ? renderEmailTemplate(preferences.emailTemplate, emailContext, 'text')
    : buildMailBodyText(requestCode, cartItems, now, preferences.categoryDisplayNames);
